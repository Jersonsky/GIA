...
<div class="container-fluid">
	<div class="card">
		<div class="card-header">
			<h4 class="card-title">Consultar Observador de Profesor: <em><?php echo $profesor -> getDocumento() . " " . $profesor -> getNombre() . " " . $profesor -> getApellido() . " " . $profesor -> getTelefono() . " " . $profesor -> getEstado() . " " . $profesor -> getCorreo() . " " . $profesor -> getClave() . " " . $profesor -> getFoto() . " " . $profesor -> getFecha_nacimiento() ?></em></h4>
		</div>
		<div class="card-body">
			...
			<table class="table table-striped table-hover">
				<thead>
					<tr><th></th>
						...
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Ascendente' href='index.php?pid=<?php echo base64_encode("ui/observador/selectAllObservadorByProfesor.php") ?>&idProfesor=<?php echo $_GET['idProfesor'] ?>&order=descripcion&dir=asc'>
							<span class='fas fa-sort-amount-up'></span></a>
						<?php } ?>
						<?php if($order=="descripcion" && $dir=="desc") { ?>
							<span class='fas fa-sort-down'></span>
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Descendente' href='index.php?pid=<?php echo base64_encode("ui/observador/selectAllObservadorByProfesor.php") ?>&idProfesor=<?php echo $_GET['idProfesor'] ?>&order=descripcion&dir=desc'>
						...
						<?php if($order=="fecha" && $dir=="asc") { ?>
							<span class='fas fa-sort-up'></span>
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Ascendente' href='index.php?pid=<?php echo base64_encode("ui/observador/selectAllObservadorByProfesor.php") ?>&idProfesor=<?php echo $_GET['idProfesor'] ?>&order=fecha&dir=asc'>
							<span class='fas fa-sort-amount-up'></span></a>
						<?php } ?>
						<?php if($order=="fecha" && $dir=="desc") { ?>
							<span class='fas fa-sort-down'></span>
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Descendente' href='index.php?pid=<?php echo base64_encode("ui/observador/selectAllObservadorByProfesor.php") ?>&idProfesor=<?php echo $_GET['idProfesor'] ?>&order=fecha&dir=desc'>
							<span class='fas fa-sort-amount-down'></span></a>
						<?php } ?>
						</th>
						<th>Profesor</th>
					...
					<?php
					$observador = new Observador("", "", "", $_GET['idProfesor'], "", "");
					if($order!="" && $dir!="") {
						$observadors = $observador -> selectAllByProfesorOrder($order, $dir);
					} else {
						$observadors = $observador -> selectAllByProfesor();
					}
					$counter = 1;
