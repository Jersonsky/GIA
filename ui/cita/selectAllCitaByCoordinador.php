...
<div class="container-fluid">
	<div class="card">
		<div class="card-header">
			<h4 class="card-title">Consultar Cita de Coordinador: <em><?php echo $coordinador -> getDocumento() . " " . $coordinador -> getNombre() . " " . $coordinador -> getApellido() . " " . $coordinador -> getTelefono() . " " . $coordinador -> getEstado() . " " . $coordinador -> getCorreo() . " " . $coordinador -> getClave() . " " . $coordinador -> getFoto() . " " . $coordinador -> getFecha_nacimiento() ?></em></h4>
		</div>
		<div class="card-body">
			...
			<table class="table table-striped table-hover">
				<thead>
					<tr><th></th>
						...
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Ascendente' href='index.php?pid=<?php echo base64_encode("ui/cita/selectAllCitaByCoordinador.php") ?>&idCoordinador=<?php echo $_GET['idCoordinador'] ?>&order=descripcion&dir=asc'>
							<span class='fas fa-sort-amount-up'></span></a>
						<?php } ?>
						<?php if($order=="descripcion" && $dir=="desc") { ?>
							<span class='fas fa-sort-down'></span>
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Descendente' href='index.php?pid=<?php echo base64_encode("ui/cita/selectAllCitaByCoordinador.php") ?>&idCoordinador=<?php echo $_GET['idCoordinador'] ?>&order=descripcion&dir=desc'>
						...
						<?php if($order=="fecha" && $dir=="asc") { ?>
							<span class='fas fa-sort-up'></span>
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Ascendente' href='index.php?pid=<?php echo base64_encode("ui/cita/selectAllCitaByCoordinador.php") ?>&idCoordinador=<?php echo $_GET['idCoordinador'] ?>&order=fecha&dir=asc'>
							<span class='fas fa-sort-amount-up'></span></a>
						<?php } ?>
						<?php if($order=="fecha" && $dir=="desc") { ?>
							<span class='fas fa-sort-down'></span>
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Descendente' href='index.php?pid=<?php echo base64_encode("ui/cita/selectAllCitaByCoordinador.php") ?>&idCoordinador=<?php echo $_GET['idCoordinador'] ?>&order=fecha&dir=desc'>
							<span class='fas fa-sort-amount-down'></span></a>
						<?php } ?>
						</th>
						<th>Coordinador</th>
					...
					<?php
					$cita = new Cita("", "", "", $_GET['idCoordinador'], "");
					if($order!="" && $dir!="") {
						$citas = $cita -> selectAllByCoordinadorOrder($order, $dir);
					} else {
						$citas = $cita -> selectAllByCoordinador();
					}
					$counter = 1;
