...
<div class="container-fluid">
	<div class="card">
		<div class="card-header">
			<h4 class="card-title">Consultar Clase de Profesor: <em><?php echo $profesor -> getDocumento() . " " . $profesor -> getNombre() . " " . $profesor -> getApellido() . " " . $profesor -> getTelefono() . " " . $profesor -> getEstado() . " " . $profesor -> getCorreo() . " " . $profesor -> getClave() . " " . $profesor -> getFoto() . " " . $profesor -> getFecha_nacimiento() ?></em></h4>
		</div>
		<div class="card-body">
			...
			<table class="table table-striped table-hover">
				<thead>
					<tr><th></th>
						...
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Ascendente' href='index.php?pid=<?php echo base64_encode("ui/clase/selectAllClaseByProfesor.php") ?>&idProfesor=<?php echo $_GET['idProfesor'] ?>&order=descripcion&dir=asc'>
							<span class='fas fa-sort-amount-up'></span></a>
						<?php } ?>
						<?php if($order=="descripcion" && $dir=="desc") { ?>
							<span class='fas fa-sort-down'></span>
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Descendente' href='index.php?pid=<?php echo base64_encode("ui/clase/selectAllClaseByProfesor.php") ?>&idProfesor=<?php echo $_GET['idProfesor'] ?>&order=descripcion&dir=desc'>
					...
					<?php
					$clase = new Clase("", "", "", $_GET['idProfesor'], "");
					if($order!="" && $dir!="") {
						$clases = $clase -> selectAllByProfesorOrder($order, $dir);
					} else {
						$clases = $clase -> selectAllByProfesor();
					}
					$counter = 1;
