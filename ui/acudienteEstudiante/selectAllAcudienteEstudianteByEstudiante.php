...
<div class="container-fluid">
	<div class="card">
		<div class="card-header">
			<h4 class="card-title">Consultar Acudiente Estudiante de Estudiante: <em><?php echo $estudiante -> getNombre() . " " . $estudiante -> getApellido() . " " . $estudiante -> getTelefono() . " " . $estudiante -> getCorreo() . " " . $estudiante -> getClave() . " " . $estudiante -> getFoto() . " " . $estudiante -> getFecha_nacimiento() . " " . $estudiante -> getDocumento() ?></em></h4>
		</div>
		<div class="card-body">
			...
			<table class="table table-striped table-hover">
				<thead>
					<tr><th></th>
						...
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Ascendente' href='index.php?pid=<?php echo base64_encode("ui/acudienteEstudiante/selectAllAcudienteEstudianteByEstudiante.php") ?>&idEstudiante=<?php echo $_GET['idEstudiante'] ?>&order=fecha&dir=asc'>
							<span class='fas fa-sort-amount-up'></span></a>
						<?php } ?>
						<?php if($order=="fecha" && $dir=="desc") { ?>
							<span class='fas fa-sort-down'></span>
						<?php } else { ?>
							<a data-toggle='tooltip' class='tooltipLink' data-original-title='Ordenar Descendente' href='index.php?pid=<?php echo base64_encode("ui/acudienteEstudiante/selectAllAcudienteEstudianteByEstudiante.php") ?>&idEstudiante=<?php echo $_GET['idEstudiante'] ?>&order=fecha&dir=desc'>
					...
					<?php
					$acudienteEstudiante = new AcudienteEstudiante("", "", $_GET['idEstudiante'], "");
					if($order!="" && $dir!="") {
						$acudienteEstudiantes = $acudienteEstudiante -> selectAllByEstudianteOrder($order, $dir);
					} else {
						$acudienteEstudiantes = $acudienteEstudiante -> selectAllByEstudiante();
					}
					$counter = 1;
